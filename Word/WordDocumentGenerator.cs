...
public sealed class WordDocumentGenerator : IDisposable {
  ...
  public WordDocumentGenerator() {
    application =
        new Word.Application { Visible = false, DisplayAlerts = Word.WdAlertLevel.wdAlertsNone };
  }
  ...
  private void GenerateOne(string templatePath, string outputDirectory, MailMergeTable table,
                           int row, Action<string, string?> report, Word.WdSaveFormat saveFormat) {
    ...
    try {
      ...
      foreach (string placeholder in table.Placeholders) {
        WordStoryReplacer.Replace(document, "{{" + placeholder + "}}",
                                  table.GetValue(placeholder, row));
      }
      if (!IsValid(document)) {

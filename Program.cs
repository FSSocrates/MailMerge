    ...
    private static int Main() {
        try {
            ...
            string templatePath = Path.GetFullPath(
                Path.Combine(AppContext.BaseDirectory, configuration.Template.Path));
            string dataPath =
                Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, configuration.Data.Path));
            string outputPath =
                Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, configuration.Output.Path));
            ...
            MailMergeTable table = new ExcelImporter().Import(
                dataPath, configuration.Data.Sheet, configuration.Data.Ranges,
            ...
            generator.Generate(templatePath, outputPath, table, (fileName, error) => {
                if (error is null) {
                    generated++;
                    Log("✔", fileName);
                } else {
